...
public partial class DnsServersManager
{
    ...
    public async Task<bool> InitializeAsync(string path, Window? window = null)
    {
        Owner = window;
        IsInitialized = false;
        bool isSaved;
        ...
        try
        {
            ...
            else if (!await XmlTool.IsValidXMLFileAsync(XmlFilePath))
            {
                if (await XmlTool.IsValidXMLFileAsync(XmlFilePath_Backup))
                {
    ...
    public static GroupSettings Create_GroupSettings(XElement? groupSettingsElement)
    {
        ...
        try
        {
            ...
            {
                if (element.Name.LocalName.Equals(nameof(groupSettings.LookupDomain)))
                {
    ...
    public async Task Create_Group_Async(string groupName, GroupMode groupMode, bool saveToFile = true)
    {
        try
        {
            ...
            XElement group = new("Group", new XAttribute("Enabled", true));
    ...
    public bool Get_Group_EnableDisable(string groupName)
    {
        bool isEnable = false;
        ...
        try
        {
            ...
            {
                XAttribute? xAttribute = groupElement.Attribute("Enabled");
                if (xAttribute != null)
                {
        ...
        {
            Debug.WriteLine("DnsServersManager Get_Group_EnableDisable: " + ex.Message);
        }

        return isEnable;
    }

    public async Task Update_Group_EnableDisable_Async(string groupName, bool newValue, bool saveToFile = true)
    {
        try
        {
            groupName = groupName.Trim();
        ...
        {
            Debug.WriteLine("DnsServersManager Update_Group_EnableDisable_Async: " + ex.Message);
        }
    }
    ...
    public List<string> Get_Group_Names(bool getOnlyEnabledGroups)
    {
        try
        {
            ...
            if (getOnlyEnabledGroups)
            {
                paths = new()
                {
                    new XmlTool.XmlPath("Group", 0, new List<XmlTool.XmlAttributeCondition>() { new("Enabled", "true") }),
                    new XmlTool.XmlPath("Name")
                };
            }
    ...
    public static GroupItem Get_GroupItem(XElement? groupElement)
    {
        ...
        try
        {
            ...
            {
                XAttribute? xAttribute = groupElement.Attribute(nameof(groupItem.Enabled));
                if (xAttribute != null)
                {
                    bool isBool = bool.TryParse(xAttribute.Value, out bool result);
                    if (isBool) groupItem.Enabled = result;
                }
            }
    ...
    public async Task<string> Add_Group_Async(XElement groupElement, bool saveToFile = true)
    {
        try
        {
