...
public class KeyValues
{
    ...
    public bool TryGetValuesByKeys(string input, out List<KeyValue> listOut)
    {
            ...
            for (int n1 = 0; n1 < list.Count; n1++)
            {
                ...
                for (int n2 = 0; n2 < list.Count; n2++)
                {
                    ...
                    if (kv1.Key.Equals(kv2.Key))
                    {
                        ...
                        {
                            isValueOk = ConsoleTools.TryGetValueByKey(input, kv1.Key, kv1.IsKeyMandatory, kv1.RequireDoubleQuotes, out string value);
                            if (!isValueOk) continue;
                            isValueOk = ConsoleTools.TryGetBool(kv1.Key, value, kv1.IsKeyMandatory, out bool result);
                            if (!isValueOk) continue;
                            kv1.Value = result;
                        ...
                        if (kv1.Type == typeof(float))
                        {
                            isValueOk = ConsoleTools.TryGetValueByKey(input, kv1.Key, kv1.IsKeyMandatory, kv1.RequireDoubleQuotes, out string value);
                            if (!isValueOk) continue;
                            isValueOk = ConsoleTools.TryGetFloat(kv1.Key, value, kv1.IsKeyMandatory, kv1.Min, kv1.Max, out float n);
                            if (!isValueOk) continue;
                            kv1.Value = n;
                            kv1.ValueFloat = n;
                            count++;
                        ...
                        if (kv1.Type == typeof(int))
                        {
                            isValueOk = ConsoleTools.TryGetValueByKey(input, kv1.Key, kv1.IsKeyMandatory, kv1.RequireDoubleQuotes, out string value);
                            if (!isValueOk) continue;
                            isValueOk = ConsoleTools.TryGetInt(kv1.Key, value, kv1.IsKeyMandatory, kv1.Min, kv1.Max, out int n);
                            if (!isValueOk) continue;
                            kv1.Value = n;
                            kv1.ValueInt = n;
                            count++;
                        ...
                        if (kv1.Type == typeof(string))
                        {
                            isValueOk = ConsoleTools.TryGetValueByKey(input, kv1.Key, kv1.IsKeyMandatory, kv1.RequireDoubleQuotes, out string value);
                            if (!isValueOk) continue;
                            isValueOk = ConsoleTools.TryGetString(kv1.Key, value, kv1.IsKeyMandatory, out value);
                            if (!isValueOk) continue;
                            kv1.Value = value;
        ...
        {
            Debug.WriteLine("KeyValue TryGetValuesByKeys: " + ex.Message);
            listOut = list;
            return false;
        }
    }
