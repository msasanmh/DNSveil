...
{
    private readonly Visual ParentVisual;
    ...
    public AppSettings(Visual visual, string? xmlFilePath = null)
    {
        ParentVisual = visual;
        if (xmlFilePath != null)
        {
    ...
    private void LoadFromXMLFile(Visual visual, string xmlFilePath)
    {
        try
        {
                                ...
                                if (control0 != null)
                                {
                                    ...
                                    if (controlPropertyExist)
                                    {
                                        ...
                                        for (int n2 = 0; n2 < controlProperties.Count(); n2++)
                                        {
                                            ...
                                            {
                                                if (IsControlExistInForm(visual, control0.Name.LocalName))
    ...
    private static bool IsControlExistInForm(Visual visual, string controlName)
    {
        List<Control> controls = Controllers.GetAllControls(visual);
    ...
    private void LoadAllSettings(Visual visual)
    {
        try
        {
            List<Control> controls = Controllers.GetAllControls(visual);
    ...
    public T? GetSettingFromList<T>(Control control, string propertyName)
    {
        ...
        {
            Debug.WriteLine("Settings GetSettingFromList: " + ex.Message);
        }
        return default;
    }
    ...
    public T? GetSettingFromXML<T>(Control control, string propertyName)
    {
        ...
        {
            Debug.WriteLine("Settings GetSettingFromXML: " + ex.Message);
        }
        return default;
    }
    ...
    {
        List<Control> controls = Controllers.GetAllControls(ParentVisual);
        for (int n1 = 0; n1 < controls.Count; n1++)
        {
            Control control = controls[n1];
            PropertyInfo[] properties = control.GetType().GetProperties();
            for (int n2 = 0; n2 < properties.Length; n2++)
            {
                ...
                {
                    Debug.WriteLine("AddAllSettings: " + ex.Message);
                }
            }
        }
    }
    ...
    /// </summary>
    public void AddSelectedSettings()
    {
        List<Control> controls = Controllers.GetAllControls(ParentVisual);
        for (int n1 = 0; n1 < controls.Count; n1++)
        {
            ...
            for (int n2 = 0; n2 < properties.Length; n2++)
            {
                ...
                {
                    Debug.WriteLine("Settings AddSelectedSettings: " + ex.Message);
                    continue;
                }
            }
        }
