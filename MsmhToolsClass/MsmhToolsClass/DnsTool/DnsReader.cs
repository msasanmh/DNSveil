...
namespace MsmhToolsClass.DnsTool
{
    public class DnsReader
    {
        ...
        {
            NetworkTool.GetUrlDetails(dns, defaultPort, out _, out string host, out int port, out string path, out bool isIPv6);
            Port = port;
            Path = path;
        ...
        {
            NetworkTool.GetHostDetails(hostIpPort, defaultPort, out string host, out int port, out string _, out bool isIPv6);
            Port = port;
            bool isIPv4 = NetworkTool.IsIPv4Valid(host, out IPAddress? _);
            if (isIPv6 || isIPv4)
            {
