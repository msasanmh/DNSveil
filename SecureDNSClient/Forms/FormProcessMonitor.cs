        ...
        private void FormProcessMonitor_Shown(object? sender, EventArgs e)
        {
            ...
            Task.Run(async () =>
            {
                while (!Exit)
                {
                    ...
                    string m05 = $"{NL} Local HTTP Proxy Address: ";
                    string m06;
                    if (FormMain.IsHttpProxyRunning)
                    {
                        m06 = $"{NL} http://{IPAddress.Loopback}:{FormMain.HttpProxyPort}{NL}";
                        m06 += $" http://{FormMain.LocalIP}:{FormMain.HttpProxyPort}{NL}";
                    }
                    else m06 = $"Offline{NL}";
